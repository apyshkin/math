...
\section{Introduction}
  ...
  \end{definition}
  %\begin{note}
    The last definition is equivalent to the existence of the \emph{linear summation method} for $\fsys$, meaning that
...
\section{B-class vector systems: rank one density problem}
    ...
    Recall that there exists a linear summation method for the system $\seq{f}$
      ...
      such that $\inner{Tf_n}{f_n^*} = 0$ for any $n$ (see Theorem 2.2 of~\cite{katavolos} for details).
    ...
    \begin{definition}
        \emph{Network} $\net$ is a quadruple $(G, \wtp, \source, \sink)$, where $G = (V, E)$ is a weighted graph
    ...
    \end{definition}
    \begin{definition}
      Let $\net = (G, \wtp, \source, \sink)$ be a network, and $\flow$ be a pseudo-flow.
    ...
    \end{definition}
    \begin{definition}
      Given a network $\net = (V, E, \wtp, \source, \sink)$ and a pseudo-flow $\flow$ we will name $\flow$ a
    ...
    Naturally, we might instead consider an oriented network $\onet = (V, \vec{E}, \wtp, \source, \sink)$ and a
      positive flow $\flowpos: \vec{E} \to \mathbb{R}^{+}$, where
    ...
    We plan to construct the network $\net(\fsys) = (V, E, \wtp, \source, \sink)$,
      length function $\wtp: E \to \mathbb{R}^{+}$ and a real skew-symmetric flow $\flow: V \times V \to \mathbb{R}$.
    ...
    Then for each $k > 0$ we connect the vertex $\source$ and the vertex $l_k$ with
      the edge $e_k = (\source l_k)$, and set the flow:
    \begin{align*}
    ...
    Assuming we have the vertices from the right part of the graph $B(\fsys)$ in the form of the sequence $\seq{r}$,
      for each $k > 0$ we connect the vertex $r_k$ to the vertex $\sink$ with the edge $e'_k=(r_k \sink)$
    ...
    Now one can see that the network $\net(\fsys)$ is $\flow$-preserving if and only if the trace of the operator $T$ is equal to
      zero.
    \begin{definition}
      Consider a network $\net = (V, E, \wtp, \source, \sink)$ and a flow $\flow$ on it.
    ...
    \begin{proof}
      ...
      Using the operator $T$, we were able to define a flow $\flow(T)$ on the network
      $\net(\fsys)$.
  ...
  \section{B-networks characterisation}
    ...
    Next we recollect some other notions from the graph theory:
    \begin{definition}
      Let $G = (V, \vec{E})$ be an oriented graph.
    ...
    \begin{proof}
      ...
      The flow $\flowpos$ as well as the length function $\wtp$ stay exactly the same (being defined on the edges set $E$).
      ...
      In that case we set the resulting flow $\flowpos(r_k r_{k+1})$ to one, creating an elusive flow
      ...
      \begin{prop}
        The properties~\ref{p1}--\ref{p5} are satisfied for the resulting preflow $\flowposn{n}$ on the network $\onet_n = (G_n, \wtp, \root)$.
      \end{prop}
      \begin{proof}
    ...
    \subsection{Application of the B-networks criterion to the Larson--Wogen example.}
      ...
      \begin{theorem}
        \label{thm-katavolos-1}
          The system $\fsys_{LW}$ admits a linear summation method if and only if the system is two point dense.
      \end{theorem}
      \begin{note}
      ...
      %  \begin{enumerate}[label=(\alph*)]
          ...
          The system $\fsys_{LW}$ admits a linear summation method if and only if
      ...
      \begin{proof}
        ...
        After that we utilize Theorem~\ref{ray-thm}, which states that there exists such
          ...
          whose total length is bounded: $ \sum_{k=1}^\infty \wtp(r_k r_{k+1}) < \infty$.
    ...
    \section{Application of the B-networks criterion to the B-class vector systems}
      ...
      \begin{remark}
        ...
        Moreover, by Theorem~\ref{thm-katavolos-1}
          there is no linear summation method for the LW system $\fsys_{LW}$ if and only if $\left\{1/a_k\right\} \in \ell^1$.
        We can conclude from these facts that the following lemma holds.
        \begin{lemma}
          \label{lw-lemma}
          Whenever the sequence $\left\{\wtp_s = 1/a_s\right\}$ is summable,
            it is possible to define a two-dimensional operator $T$ in such a form:
          \begin{align*}
            &T_{11} = 1,\\
            &T_{kk} = 0, \quad k > 1,\\
            &T_{2k-1, 2k} = \wtp_{2k-1},\\
            &T_{2k+1, 2k} = \wtp_{2k}.
          \end{align*}
        \end{lemma}
      \end{remark}
      ...
      \end{theorem}
      \begin{proof}
        The necessity is trivial considering that the two point density property is weaker than the existence of the linear summation method for $\fsys$.

        Now let us prove the sufficiency.
        Presume that $\fsys$ does not admit a linear summation method.
        ...
        \begin{align*}
          &T'_{11} = 1,\\
          &T'_{kk} = 0,\\
          &T'_{2k-1, 2k} = \wtp(r_{2k-1} r_{2k}) \flowpos(r_{2k-1} r_{2k}) = \wtp(r_{2k-1} r_{2k}),\\
          &T'_{2k+1, 2k} = \wtp(r_{2k+1} r_{2k}) \flowpos(r_{2k+1} r_{2k}) = \wtp(r_{2k+1} r_{2k}).
        \end{align*}
        Notice that, since the ray $r$ has a finite length, $\wtp(r_k r_{k+1})$ is a summable sequence.
