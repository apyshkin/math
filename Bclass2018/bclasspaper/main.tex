...
\section{Introduction}
  ...
  In this paper we are going to analyze the rank one density property~\ref{r1d} for this class.
    ...
    \begin{theorem}
      The B-class system $\fsys$ is rank one dense if and only if
        for any infinite path $\seq{r}$ in the bipartite graph $B(\fsys)$
          the series $\sum_{k=1}^\infty\lvert\wt^{-1}(r_k, r_{k+1})\rvert$ diverges.
    \end{theorem}
    \begin{figure}
      \begin{center}
