...
\section{B-class of vector systems}
  ...
  \subsection{B-networks characterisation}
    ...
    \begin{proof}
      ...
      However by the definition of B-networks the sink is also connected to the source with the path of a finite length.
      ...
      \begin{proof}
        ...
        \]
        Since the sequence $\{\flowposn{n}(e)$ is decreasing and bounded from below the quantity $d_{\preflow}(\root)$ is defined correctly.
        Because $d_{\flowposn{n}}(\root)$ is equal to $d_\flowpos(\root)$ for any $n$, we have
        \[
      ...
      \begin{remark}
        ...
        \]
        If we think about the network $\onet$ as a rooted tree (see Figure \alex{a figure with correspondence: rooted trees <-> oriented networks}) with a root $\root$
      ...
      \begin{note}
        ...
        See the figure xxx.
        \alex{a figure with $n$-th layer and BFS}
      \end{note}
      ...
      Notice that the end vertices of such edges $\eback_{n}$ now became $\cal{G}$-active, since for such vertices
        the total incoming flow now exceeds the total outgoing flow. \alex{a figure of a back edge making some inner vertex non-preserving}
    ...
    \subsubsection{Application of the B-networks criterion to the Azoff--Shehada example.}
      ...
      \begin{proof}
        ...
        After that we utilize the Theorem~\ref{ray-thm}, which states that there exist such
          ...
          \]
          \alex{a figure with the AS ray in AS network}
