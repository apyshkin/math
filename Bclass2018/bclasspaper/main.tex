...
\section{B-class vector systems: rank one density problem}
  ...
  \section{B-networks characterisation}
    ...
    \begin{proof}[Proof of Theorem~\ref{thm-main}]
      The system $\fsys$ belongs to B-class, so we could apply Theorem~\ref{thm-graph-eq}, which asserts
      ...
      We could rephrase the last conclusion in terms of \emph{oriented} networks.
      ...
      After that we utilize Theorem~\ref{ray-thm}, which states that there exists such
        ...
        the network $\onet(\fsys)$, if and only if there exists a ray $r^{*} \subseteq \onet$, originating in the vertex $\source$,
        whose total length is bounded: $\sum_{k=1}^\infty \len(r^{*}_k r^{*}_{k+1}) < \infty$.
      Combining these two statements we get that $\fsys$ is \emph{not} rank one dense when and only when there is a finite-length ray $r^*$ in the network $\onet(\fsys)$.
      ...
      Therefore, such $r^*$ exists in $\onet(\fsys)$ if and only if there is a ray $r=\seq{r}$ in the original bipartite graph $B(\fsys)$ such that
        the series $\sum_{k=1}^\infty\lvert\wt^{-1}(r_k, r_{k+1})\rvert$ converges.
