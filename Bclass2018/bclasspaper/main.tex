...
\section{B-class of vector systems}
  \subsection{From vector system to the countable network: linear summation problem}
    ...
    \begin{definition}
        ...
        \begin{enumerate}[label={(\Alph*)}]
            ...
            \item \label{d3} $\inner{f_n}{e_n} = \inner{f^*_n}{e_n} = 1$ for any $n > 0$,
            \item \label{d4} $\inner{f_n}{e_k} = -\inner{f^*_k}{e_n}$ for any $n, k > 0; n \neq k$,
    ...
    \begin{proof}
      ...
      The property~\ref{d4} yields that $\inner{f_n}{e_k}$ is equal to zero whenever $k$ is not in
        $\mathbb{N} \cup {n}$.
      ...
      \]
      if $n \neq m$.
      Otherwise if $n = m$ we get $\inner{f_n}{f^*_m} = 1$ due to the property~\ref{d4}.
    \end{proof}
    ...
    \end{definition}
    \begin{definition}
        Look at the skew-symmetric function $\flow: V \times V \to \mathbb{R}$.
  ...
  \subsection{B-class networks characterisation}
    ...
    \begin{theorem}
      \label{ray-thm}
      Let $\onet = (G, \wtp, \source, \sink)$ be an oriented B-class network.
    ...
    \begin{proof}
      ...
      You can see an example of positive flow cycle in the picture~\ref{positive-flow}.
      \begin{figure}
        \begin{center}
      ...
      \begin{remark}
        ...
        \]
        If we think about the network $\onet$ as a rooted tree (see picture \alex{a picture with correspondence: rooted trees <-> oriented networks}) with a root $\root$
      ...
      \begin{note}
        ...
        See the figure xxx.
        \alex{a picture with $n$-th layer and BFS}
      \end{note}
      ...
      \begin{note}
        ...
        \alex{a picture of the first layer (infinite degree!)}
      ...
      \alex{a picture of the $n$-th layer construction}
      ...
      Notice that the end vertices of such edges $\eback_{n}$ now became $\cal{G}$-active, since for such vertices
        the total incoming flow now exceeds the total outgoing flow. \alex{a picture of a back edge making some inner vertex non-preserving}
    ...
    \subsubsection{Application of the B-class networks criterion to the Azoff--Shehada example.}
      ...
      \begin{proof}
        ...
        After that we utilize the theorem~\ref{ray-thm}, which states that there exist such
          ...
          \]
          \alex{a picture with the AS ray in AS network}
      ...
      However, we already found the condition for the linear summation method existence, namely
        there is no linear summation method for the AS system $\fsys_{AS}$ iff $\left\{1/a_k\right\}$
