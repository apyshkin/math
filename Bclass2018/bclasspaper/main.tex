...
\section{Introduction}
  ...
  %\begin{remark}
    The last definition is equivalent to the defintion of the \emph{linear summation method} existence for system $\fsys$, namely that
      ...
      \[
        \lim_{n \to \infty} \sum_k \alpha_{kn} \inner{x}{f^*_k} = x,
      \]
      for any $x \in \cal{H}$.
  %\end{remark}
  ...
  This example was also studied in~\cite{argyroslambrou} (see Addendum) and by Azoff and Shehada in~\cite{azoff} regarding
  ...
  We are wondering whether the two point density always implies the rank one density property for any $M$-basis $f_k$.
...
\section{B-class vector systems: rank one density problem}
    ...
    Furthermore, it appears that the B-class vector system could be restored using an infinite bipartite graph.
    \begin{remark}
      ...
      We set the weight on the edge $(lr)$ as follows: $\wt_{lr} = \inner{f_l}{e_r}^{-1}$.
    ...
    Our purpose is to analyze the rank one density property of $\fsys$.
    ...
    \begin{align*}
      &\flow(v_l, \source) = -\flow(\source, v_l) = T_{v_l v_l}.
    \end{align*}
    ...
    \begin{align*}
      &\flow(v_r, \sink) = -\flow(\sink, v_r) =  T_{v_r v_r}.
    \end{align*}
    ...
    The total flow $d(\source)$ in the source vertex is equal to $\sum_{v_l} T_{ll}$, and the total flow in the
      sink vertex is now equal to $\sum_{v_r} T_{rr}$.
    ...
    \end{definition}
    %\begin{definition}
      %If $\flowpos$ is an oriented flow on the oriented network $\onet = (V,\vec{E},\len, \source, \sink)$ 
  ...
  \section{B-networks characterisation}
    ...
    It essentially equips us with tools to analyze the 
    \begin{theorem}
      \label{ray-thm}
      Let $\onet = (\vec{G}, \len, \source, \sink)$ be an oriented B-network.
      Then there exists such a positive flow $\flowpos: \vec{E} \to \mathbb{R}^{+}$ that
    ...
    \begin{proof}
      ...
      In order to make our proof easier, we consider a modified network $\onet' = (\vec{G}', \source', \source')$.
      ...
      In that case the graph $\vec{G'}$ is a rooted graph with the root at the vertex $\source'$.
      ...
      The flow $\flowpos$ as well as the length function $\len$ stay exactly the same (being defined on the edges set $\vec{E}'$).
      ...
      Thus $\onet'$ is a network with a function $\flowpos$ being a flow as required.
      Obviously the mass of the flow does not change since we did not change the lengths of the edges.
      Furthermore the network $\onet'$ does not preserve the flow $\flowpos$ by construction.

      It is clear that $\onet'$ contains a finite length ray starting on the vertex $s'$ if and only if the original network $\onet$ contains a finite
      ...
      \begin{theorem*}
        Let $\onet = (G, \len, \source, \sink)$ be an oriented B-network such that $\source = \sink$.
      ...
      All the other edges will not carry any flow, hence the flow is preserved
        at each vertex but not preserved by the whole network $\onet$.
      ...
      At each step $n \in \mathbb{N}$ we examine a subgraph $G_n \subset G$ and present a flow $\flowposn{n}: \onet \to \mathbb{R}^{+}$
      ...
      Suppose we have already established the flow $\flowposn{n}: \onet \to \mathbb{R}^{+}$ for which
      ...
      Take the minimal value the flow $\cal{E}_0$ attains on the edges of $C_1$:
        \[
      ...
      Then set the flow $\flowposn{n+1}$ equal to the final flow $\cal{E}_K$:
      \[
      ...
      Consider now the preflow $\preflow$ defined as a pointwise limit of $\flowposn{n}$:
      \[
      ...
      \end{proof}
      \begin{prop}
        Consider the flow $\preflow$ on the network $\onet$.
      ...
      \begin{definition}
        Let $G = (V, \vec{E},\len, \root)$ be a weighted directed graph with a positive
          length function $\len$ defined on $\vec{E}$ and a root vertex $\root \in V$.
      ...
      Conclusively, we set the preflow $\flowposn{n}$ equal to the $\cal{G}$ we got after relaxations.
      \begin{prop}
        The properties~\ref{p1}--\ref{p5} are satisfied for the resulting preflow $\flowposn{n}$ on the network $\onet_n = (G_n, \len, \root)$.
      \end{prop}
      \begin{proof}
      ...
      \begin{proof}
        ...
        \[
          \phi_n(u) \overset{\mathit{def}}{=} \inf_{p \in \gnpaths(u)} \phi_n(p) \leq \phi_n(v) + \len(vu).
        \]
        It implies that
        \[
          LS\leq \sum_{(vu) \in E_n} \flowposn{n}(vu) \len(vu) = \sum_{e \in E_n} \flowposn{n}(e) \len(e).
        \]
        Due to the property~\ref{p2} we get the following:
        \[
          LS\leq \sum_{e \in E_n} \flowposn{n}(e) \len(e) \leq \sum_{e \in E_n} \flowpos(e) \len(e).
        \]
      ...
      \begin{proof}
        ...
        Since $\sum\limits_{v \in L_n} d^{-}_n(v)$ is the total amount of the flow incoming to the deepest layer $L_n$, and the only
          active vertex in $G_n$ is the root, which produces the flow which size is equal to $1$, the following is true:
        \[
        ...
        \[
          \min_{L_n} \phi_n(v) \leq LS \leq \sum_{e \in E_n} \len(e) \flowpos(e) \leq \lvert\flowpos\rvert.
        \]
      ...
      \begin{proof}
        ...
        Each $U_n$ is non-empty and if the vertex $u$ belongs to $U_n$ then there always exists
        ...
        The sum $\sum \len(r_k r_{k+1})$ is bounded since the partial sums are bounded by the values of the function $\phi_n$
  ...
  \section{Application of the B-networks criterion}
    \subsection{Application of the B-networks criterion to the Larson--Wogen example.}
      The Larson--Wogen system is given in the Example~\ref{lw-sys}.
      The sequences $\{f_j\}$, $\{f^*_j\}$ are biorthogonal and both are complete in $\cal{H}$ (thus $\fsys$ is a $M$-basis).
      Also due to Katavolos, Lambrou and Papadakis~\cite{katavolos} the next two theorems hold.
      \begin{theorem}
        \label{thm-katavolos-1}
          The system $\fsys_{LW}$ admits a rank one density property if and only if the system is two point dense.
      \end{theorem}
      \begin{remark}
      ...
      \begin{proof}
        Notice that $\fsys=\fsys_{LW}$ belongs to B-class.
        Thus Theorem~\ref{thm-graph-eq} applies to the system $\fsys$, which asserts
          that $\fsys$ admits the rank one density property if and only if
          the network $\net(\fsys)$ is $\flow$-preserving for any
          finite-mass flow $\flow$.
        We could rephrase the conclusion in terms of \emph{oriented} networks.
        ...
        After that we utilize Theorem~\ref{ray-thm}, which states that there exists such
          ...
          if and only if there exists a ray $r = \seq{r} \subseteq \onet $ originating in the vertex $\source$,
          whose total length is bounded: $ \sum_{k=1}^\infty \len(r_k r_{k+1}) < \infty$.
          Hence, the system is not rank one dense when and only when there is such ray $r$
            with the properties specified up above.

          How does look the ray of a finite length in the network like $\onet(\fsys)$?
          It is obvious that it cannot include infinite number of the edges incident to the vertices
          ...
          However this ray has the length equal to $\sum \len(r_k r_{k+1}) = \sum 1/\lvert a_k \rvert$,
