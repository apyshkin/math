...
\section{B-class of vector systems}
  \subsection{From vector system to the countable network: linear summation problem}
    ...
    \begin{definition}
      ...
      \begin{align*}
        &d^{+}_{\flow}(v) = d^{+}(v) := \sum_{u \mid \flow(vu) > 0} \flow(vu),\\
        &d^{-}_{\flow}(v) = d^{-}(v) := \sum_{u \mid \flow(vu) < 0} \flow(uv).
      \end{align*}
    ...
    \begin{align*}
      &d^{+}_{\flowpos}(v) = d^{+}(v) := \sum_{u \in \eout(v)} \flowpos(vu),\\
      &d^{-}_{\flowpos}(v) = d^{-}(v) := \sum_{u \in \ein(v)} \flowpos(uv),\\
      &d_{\flowpos}(v) = d(v) := d^{+}(v) - d^{-}(v).
    \end{align*}
    \begin{definition}
    ...
    Then for each $k > 0$ we connect the $\source$ vertex $s$ and the $l_k$ vertex with
