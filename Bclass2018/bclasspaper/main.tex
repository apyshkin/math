...
\newcommand{\preflow}{\mathcal{\hat{F}}_{pre}}
...
\section{B-class of vector systems}
  ...
  \subsection{B-class networks characterisation}
    ...
    \begin{definition}
      ...
      A sequence of vertices $\{v_k\}_{k=1}^N$ is called a \emph{path} (simple path) if each consequtive vertices
    ...
    \begin{definition}
      ...
      An infinite path $r = \seq{v}$ is called a \textit{ray} iff
  ...
  \subsection{Application of the B-class networks characterisation}
    ...
    \begin{proof}
      ...
      \noindent\textit{Sufficiency}
      Pretend $\fsys$ admits a linear summation method.
      Let $\onet = \onet(\fsys) = (G, \wtp, \source, \sink)$ be an orientation of the network $\net(\fsys)$.
      Applying the theorem~\ref{thm-graph-eq} and then the theorem~\ref{ray-thm}
        we recognize that any ray $r \subseteq \onet$ taking its origin
        in the source vertex \source has an infinite weight.
      Suppose $r = \sys{r}$ such that $r_0 = \source$ and $r_k$ are some vertices in the graph $G$.
      ...
      \]
      such that $\cal{H}_1 = \span(\bigcup\limits_k e_{s_k})$.
