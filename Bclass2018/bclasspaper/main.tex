...
\section{B-class of vector systems}
  ...
  \subsection{B-class networks characterisation}
    ...
    \begin{proof}
      ...
      \begin{itemize}
        \item each outcoming edge has a positive flow: $\cal{G}(e) > 0$ for any $e \in \eout(u)$,
      ...
      \end{itemize}
      \begin{note}
        Literally, here we are pushing the flow out of the vertex $u \in L_{n-1}$ spreading it among all the outcoming edges
          $\eout_G(u)$.
      ...
      So if we had no back edges, the function $\cal{G}$ would be our next preflow function $\flowposn{n}$, and this step would be accomplished,
        since all the properties~\ref{p1}--\ref{p2} would be complied by $\cal{G}$.
      ...
      Notice that the end vertices of such edges $\eback_{n}$ now became $\cal{G}$-active, since for such vertices
        the total incoming flow now exceeds the total outcoming flow.
      ...
      Consider a $\cal{G}$-active vertex $s \in \ter{\eback_n}$ such that $d^{\cal{G}}_{-} > d^{\cal{G}}_{+}$.
