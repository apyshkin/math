...
\section{B-class of vector systems: linear summation problem}
    ...
    \begin{figure}
      ...
      \begin{tikzpicture}[thick,
        ...
        \end{scope}
      \end{tikzpicture}
      \caption{The bipartite graph $B(\fsys_{AS})$ } \label{as-bgraph}
      \end{center}
    \end{figure}

    You can see the bipartite graph built from the Azoff--Shehada system on the Figure~\ref{as-bgraph}.
  ...
  \section{B-networks characterisation}
    ...
    \begin{definition}
      Let $G = (V, \vec{E})$ be an oriented graph.
      A sequence of vertices $\{v_k\}_{k=1}^N$ is called a \emph{path} if each consecutive vertices
  ...
  \section{Application of the B-networks criterion}
    \subsection{Application of the B-networks criterion to the Azoff--Shehada example.}
      ...
      \end{prop}
      \begin{proof}
        Here we will discuss the Azoff-Shehada system only: $\fsys = \fsys_{AS}$.
    ...
    \begin{remark}
      Concerning the linear summation method property, the last theorem reveals that the B-class vector systems
        admit the same criterion as the Azoff-Shehada system.
      Furthermore the Azoff-Shehada system is to a certain extent a canonical element of this class,
        since the corresponding bipartite graph $B(\cal{F}_{AS})$
        is a ray itself (see Figure~\ref{as-bgraph}).
    \end{remark}
\bigskip
\include{bib}
\vspace{1em}
