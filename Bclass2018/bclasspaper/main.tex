\documentclass[12pt,oneside,a4paper,draft]{amsart}
\input{preamble}
\usepackage{caption}
\usepackage{tikz}
...
\section{Introduction}
  ...
  Let ${\cal{A} = \{T\in B(\cal{H}): Tf_n \in \overline{span}(f_n), n \in \mathbb{N}\}}$ be an operator algebra,
  ...
  That is a common fact that the last definition is equivalent to $R_1(\cal{A})$ being dense
    in $\cal{A}$ in the ultraweak (or $\sigma$-weak) topology.
  It extends the notion of \emph{linear summation method} for the system $\fsys$, namely
    such $\alpha_{kn} \in \mathbb{C}$ that for any $x \in \cal{H}$,
    \[
  ...
  In the next sections we are going to study the specific class of vector systems that represents a natural generalization of
    the Larson--Wogen vector system, particularly we are going to analyze the rank one density property~\eqref{r1d} for this class.
    ...
    \begin{proof}
      ...
      \subsection{Eliminating the positive flow cycles}
        ...
        \begin{prop}
          ...
          Also there are no positive flow cycles of $\preflow$.
        \end{prop}
        \begin{proof}
          ...
          Conclusively, the function $\preflow$ is a flow in the network $\net$.
...
%            $\delta \leq d_{\cal{G}}^{-}(s) - d_{\cal{G}}^{+}(s) = -d_{\cal{G}}(s)$.
        ...
        \subsection{Estimates on the constructed preflow}
          ...
          \begin{proof}
            ...
            Consider such paths $p$, starting on the root, such that $\phistar(p) \leq \lvert\flowpos\rvert$.
            Denote this set of paths by $\cal{P}^*$.

            For each $k$ we are going to present a vertex $r_k$ such that there are an infinite number of paths
              in $\cal{P}^*$ starting on $(r_1 r_2 \dots r_k)$.

            For the first step we take the root vertex $r_1 = \root$.
