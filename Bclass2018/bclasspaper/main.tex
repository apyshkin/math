...
\section{B-class of vector systems}
  \subsection{From vector system to the countable network: linear summation problem}
    ...
    \begin{definition}
      ...
      \begin{align*}
          d^{+}(v) = \sum_{u \in \eout(v)} \flow(vu),\\
          d^{-}(v) = \sum_{u \in \ein(v)} \flow(uv).
      \end{align*}
    ...
    \begin{proof}
      ...
      Besides we are going to establish a sequence of the positive preflow functions $\flowposn{n}: \vec{E}_n \to \mathbb{R}^{+}$,
        ...
        \begin{enumerate}[label=\textbf{P\arabic*}]
          ...
          \item \label{p5} the total flow of $\flowposn{n}$ of the vertex $v \in L_n$ : $d_{G_n}(v) = -d^{-}_{G_n}(v) < 0$.
        \end{enumerate}
      ...
      Since $d^{\cal{G}}_{-}(u) \leq d^{\flowpos}_{-}(u)$ (incoming flow $\cal{G}$ is bounded above by the original flow $\flowpos$),
      ...
      Let there exist such path $p \in \gpaths(s, L_n)$, connecting the vertex $s$ and the $n$-th layer, that
      ...
      \end{definition}
      For each path $p \in \gpaths(s, L_n)$ we raise the preflow $\cal{G}$ at every edge of the path $p$
      by $\delta > 0$.
      ...
      In this case we push the flow $\delta$ along the path $p$ and look for the other augmenting in $\gpaths(s, L_n)$.
      ...
      \end{prop}
      \begin{proof}
        Firstly, we have a finite graph $G_{n}$ so there are only a finite number of paths in the $\gpaths(s, L_n)$.
        ...
        Since the number of the edges is finite this must stop after some number of steps.

        Suppose that at this time the vertex $s$ is not $\cal{G}$-relaxed.
        Examine the set of vertices in the graph $G_n$ which are reachable by non-saturated paths from the vertex $s$
        (non-saturated path here is the path on which there is no $\cal{G}$-saturated edge).
        Entitle this set $V_s$.
        Obviously $V_s \cap L_n = \emptyset$.
        Now observe the subgraph $U \subseteq G_n$:
        \[
          U := \bigcup \left\{p \in \gpaths(s, V_s) \mid \text{$p$ is not $\cal{G}$-saturated})\right\},
        \]
        where $p$ is a non-saturated path from $s$ to $V_s$.
        ...
        It also does not have outgoing non-saturated edges in the whole graph $G$ since all the outgoing edges it had in $G$
          ought to have been included into the graph $G_n$.
        Having a vertex $u \in V_u$ we certainly have that $d_{\cal{G}}(u) = d^{+}_{\cal{G}}(u) - d^{-}_{\cal{G}}(u) \leq 0$ because
      ...
      Suppose we got an updated preflow function $\cal{G}$ on the network $\net_N = (G_n, \wt, \root)$.
      \begin{prop}
        The properties~\ref{p1}--\ref{p5} are satisfied.
      \end{prop}
      \begin{proof}
        ...
        The root vertex was could not be affected during the step $n > 1$, so~\ref{p3} holds.
